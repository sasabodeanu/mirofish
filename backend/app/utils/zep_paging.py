...
Zep's node/edge list API uses UUID cursor-based paging.
This module wraps the auto-pagination logic (including per-page retry) to transparently return a complete list to callers.
"""
...
def fetch_all_nodes(
    ...
    """Paginate through graph nodes, returning at most max_items items (default 2000). Each page request has built-in retry."""
    all_nodes: list[Any] = []
...
def fetch_all_edges(
    ...
    """Paginate through all graph edges, returning a complete list. Each page request has built-in retry."""
    all_edges: list[Any] = []
    cursor: str | None = None
    page_num = 0
