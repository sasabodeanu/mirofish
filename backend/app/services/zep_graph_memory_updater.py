...
class AgentActivity:
    ...
    def to_episode_text(self) -> str:
        ...
        No simulation-related prefix to avoid misleading graph updates
        """
        ...
        # Return in "agent_name: activity_description" format directly, without simulation prefix
    ...
    def _describe_quote_post(self) -> str:
        ...
        base = ""
        if original_content and original_author:
            base = f"quote-posted {original_author}'s post '{original_content}'"
        elif original_content:
            base = f"quote-posted a post '{original_content}'"
        elif original_author:
            base = f"quote-posted a post by {original_author}"
        else:
            base = "quote-posted a post"
...
class ZepGraphMemoryUpdater:
    ...
    Groups by platform; sends to Zep in batches after accumulating BATCH_SIZE activities.
    
    All meaningful actions are updated to Zep; action_args include complete context info:
    - Original content of liked/disliked posts
    - Original content of reposted/quoted posts
    - Usernames of followed/muted users
    ...
    # Send interval (seconds), avoid rate limiting
    SEND_INTERVAL = 0.5
    ...
    def __init__(self, graph_id: str, api_key: Optional[str] = None):
        ...
        # Per-platform activity buffer (each platform accumulates to BATCH_SIZE before batch send)
    ...
    def add_activity(self, activity: AgentActivity):
        ...
        action_args will contain complete context information (e.g., original post content, username, etc.).
    ...
    def _worker_loop(self):
        ...
        while self._running or not self._activity_queue.empty():
            try:
                # Try to get activity from queue (timeout 1s)
                try:
