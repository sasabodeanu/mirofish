...
This script reads parameters from config files to execute simulation, fully automated
...
Usage:
    ...
    python run_twitter_simulation.py --config /path/to/simulation_config.json --no-wait  # close immediately on completion
"""
...
class TwitterSimulationRunner:
    ...
    def _get_active_agents_for_round(
        ...
        # Calculate activation probability from each Agent's config
        candidates = []
...
def setup_signal_handlers():
    """
    Set up signal handler to ensure correct exit on SIGTERM/SIGINT
    Allows program to clean up resources (close database, environment, etc.)
    """
    def signal_handler(signum, frame):
        ...
        if not _cleanup_done:
            ...
            # Only force exit if signal received again
            print("Forcing exit...")
            sys.exit(1)
