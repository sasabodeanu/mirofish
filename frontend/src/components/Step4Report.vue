            ...
            >
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <!-- Section Content Generated (content generated, but full section may not be done yet) -->
                  ...
                  <!-- Section Complete (section fully generated) -->
                  ...
                  <template v-if="log.action === 'llm_response'">
                    ...
                    <!-- Special hint when this is the final answer -->
...
const toggleRawResult = (timestamp, event) => {
  ...
  // Wait for DOM update, then adjust scroll to keep button in same viewport position
...
const parseInsightForge = (text) => {
  ...
  try {
    ...
    // Extract core entities - full extraction, including summaries and related fact counts
...
const parseInterview = (text) => {
  ...
  try {
    ...
    // Extract selection reasons for interview subjects
    const reasonMatch = text.match(/### Interview Subject Selection Reasons\n([\s\S]*?)(?=\n---\n|\n### Interview Records)/)
    if (reasonMatch) {
      result.selectionReason = reasonMatch[1].trim()
    }
    
    // Parse selection reasons for each person
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      if (answerMatch) {
        ...
        if (!twitterMatch && redditMatch) {
          // Only Reddit answer: copy as default if not placeholder
          ...
          // No platform marker (very old format), use full text as answer
    ...
    return () => h('div', { class: 'insight-display' }, [
      ...
      h('div', { class: 'insight-content' }, [
        ...
        activeTab.value === 'subqueries' && props.result.subQueries.length > 0 && h('div', { class: 'subqueries-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Drift Queries: Derived Sub-questions'),
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    const isPlaceholderText = (text) => {
      if (!text) return true
      const t = text.trim()
      return t === '(No response from this platform)' || t === '[No reply]'
    }
    ...
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // 1. "QuestionX:" format (new backend format)
      // 2. "1. " or "\n1. " (number+dot, legacy compatibility)
      let matches = []
      let match
    ...
    // Check if a question has real dual-platform answers (filter placeholders)
...
const renderMarkdown = (content) => {
  ...
  // Remove leading h2 headings (## xxx) since section title is already shown outside
  ...
  // Handle lists - supports sub-lists
  ...
  // Clean whitespace after list open tag
  html = html.replace(/<ul class="md-ul">\s+/g, '<ul class="md-ul">')
  html = html.replace(/<ol class="md-ol">\s+/g, '<ol class="md-ol">')
  // Clean whitespace before list close tag
  ...
  // Clean <p><br> immediately before block elements (from excess blank lines)
  html = html.replace(/<p class="md-p">(<br>\s*)+(<ul|<ol|<blockquote|<pre|<hr)/g, '$2')
  // Clean consecutive <br> tags
  html = html.replace(/(<br>\s*){2,}/g, '<br>')
  // Clean <br> before paragraph start tag that follows block elements
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // Fix numbering for non-contiguous ordered lists: when single-item <ol> is separated by paragraph content, maintain incrementing numbers
...
const extractFinalContent = (response) => {
  ...
  // Try to find content after "Final Answer:" in Chinese format
  ...
  // If content is long with markdown formatting, try removing thought process before returning
