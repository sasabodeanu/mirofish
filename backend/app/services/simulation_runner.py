...
class SimulationRunner:
    ...
    Responsible for:
    1. Running OASIS simulation in background processes
    ...
    def start_simulation(
        ...
        try:
            ...
            # Create main log file to avoid process blocking from full stdout/stderr pipe buffer
            main_log_path = os.path.join(sim_dir, "simulation.log")
            main_log_file = open(main_log_path, 'w', encoding='utf-8')
            
            # Set subprocess environment variables to ensure UTF-8 encoding on Windows
            # This fixes the issue of third-party libraries (e.g., OASIS) reading files without specifying encoding
            env = os.environ.copy()
                        ...
                        try:
                            ...
                            if "event_type" in action_data:
                                ...
                                # Update round info (from round_end event)
                                    ...
                                    # Overall rounds take the maximum of both platforms
    ...
    def _check_all_platforms_completed(cls, state: SimulationRunState) -> bool:
        ...
        Platform enabled status is determined by checking if corresponding actions.jsonl file exists
    ...
    def get_all_actions(
        ...
        # Read Twitter action file (platform automatically set to twitter based on file path)
        twitter_actions_log = os.path.join(sim_dir, "twitter", "actions.jsonl")
        if not platform or platform == "twitter":
            actions.extend(cls._read_actions_from_file(
                twitter_actions_log,
                default_platform="twitter",  # automatically populate platform field
                platform_filter=platform,
                agent_id=agent_id, 
                round_num=round_num
            ))
        
        # Read Reddit action file (platform automatically set to reddit based on file path)
        reddit_actions_log = os.path.join(sim_dir, "reddit", "actions.jsonl")
        if not platform or platform == "reddit":
            actions.extend(cls._read_actions_from_file(
                reddit_actions_log,
                default_platform="reddit",  # automatically populate platform field
                platform_filter=platform,
                agent_id=agent_id,
                round_num=round_num
            ))
    ...
    def cleanup_simulation_logs(cls, simulation_id: str) -> Dict[str, Any]:
        """
        Clean up simulation run logs (used for force restart of simulation)
    ...
    def register_cleanup(cls):
        ...
        Called on Flask application startup to ensure all simulation processes are cleaned up on server shutdown
        """
        ...
        # If not in debug mode, this env var is absent, but registration is still needed
        is_reloader_process = os.environ.get('WERKZEUG_RUN_MAIN') == 'true'
        is_debug_mode = os.environ.get('FLASK_DEBUG') == '1' or os.environ.get('WERKZEUG_RUN_MAIN') is not None
        
        # In debug mode, register only in reloader subprocess; in non-debug mode, always register
        if is_debug_mode and not is_reloader_process:
            _cleanup_registered = True  # mark as registered to prevent subprocess from registering again
            return
        ...
        try:
            ...
            logger.warning("Unable to register signal handlers (not in main thread), using atexit only")
    ...
    def check_env_alive(cls, simulation_id: str) -> bool:
        ...
        Returns:
            True if environment is alive, False if environment has closed
        """
        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
        if not os.path.exists(sim_dir):
            return False
    ...
    def interview_agent(
        ...
        Args:
            ...
            platform: specify platform (optional)
                ...
                - None: in dual-platform simulation, interview both platforms simultaneously and return combined results
    ...
    def interview_agents_batch(
        ...
        Args:
            ...
            platform: default platform (optional, overridden by individual interview item platform)
    ...
    def close_simulation_env(
        ...
        """
        Shut down simulation environment (without stopping simulation process)
        
        Send environment shutdown command to simulation for graceful exit from command-waiting mode
        ...
        logger.info(f"Sending environment shutdown command: simulation_id={simulation_id}")
        ...
        try:
            ...
            return {
                "success": True,
                "message": "Environment shutdown command sent (response timeout, environment may be closing)"
            }
