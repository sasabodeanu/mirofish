    ...
    <div class="graph-container" ref="graphContainer">
      ...
      <div v-if="graphData" class="graph-view">
        ...
        <div v-if="showSimulationFinishedHint" class="graph-building-hint finished-hint">
          ...
          <span class="hint-text">A small amount of content is still processing. Manual graph refresh recommended shortly.</span>
...
const wasSimulating = ref(false) // Track whether simulation was active

// Dismiss simulation-ended hint
...
const renderGraph = () => {
  ...
  // Process edge data: count and index edges between same node pairs
  const edgePairCount = {}
  const selfLoopEdges = {} // Self-loop edges grouped by node
  const tempEdges = edgesData
    .filter(e => nodeIds.has(e.source_node_uuid) && nodeIds.has(e.target_node_uuid))
  
  // Count edges per node pair and collect self-loop edges
  tempEdges.forEach(e => {
  ...
  // Track edge index per node pair
  const edgePairIndex = {}
  ...
  tempEdges.forEach(e => {
    ...
    // Compute curvature: spread apart for multiple edges, straight for single
    let curvature = 0
    if (totalCount > 1) {
      ...
      // If edge direction is reversed relative to normalized, flip curvature
      // Ensures all edges are distributed in the same reference frame
      if (isReversed) {
        curvature = -curvature
      }
    }
  ...
  const getLinkPath = (d) => {
    ...
    if (d.isSelfLoop) {
      ...
      const loopRadius = 30
      // Start from the right side of the node, loop around
    ...
    // Compute curve control point - dynamic based on edge count and distance
    const dx = tx - sx, dy = ty - sy
    const dist = Math.sqrt(dx * dx + dy * dy)
    // Perpendicular offset based on distance ratio, ensures curve is visible
  ...
  // Link labels background (white background for clarity)
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .call(d3.drag()
      ...
      .on('drag', (event, d) => {
        // Detect if real drag started (moved beyond threshold)
        const dx = event.x - d._dragStartX
        const dy = event.y - d._dragStartY
        const distance = Math.sqrt(dx * dx + dy * dy)
        
        if (!d._isDragging && distance > 3) {
          // Only restart simulation on first detection of real drag
          d._isDragging = true
      ...
      .on('end', (event, d) => {
        // Only let simulation decelerate if a real drag occurred
        if (d._isDragging) {
