<template>
  ...
  >
    <!-- Background decoration: technical grid lines (only shown when projects exist) -->
        ...
        <div v-if="selectedProject" class="modal-overlay" @click.self="closeModal">
          <div class="modal-content">
            ...
            <div class="modal-playback-hint">
              <span class="hint-text">Step3 "Run Simulation" and Step5 "Deep Interaction" must be started live and do not support historical replay</span>
            </div>
          </div>
        </div>
      </Transition>
...
let pendingState = null  // Track pending target state
...
const containerStyle = computed(() => {
  ...
  // Expanded state: compute height based on card count
...
const getCardStyle = (index) => {
  ...
  if (isExpanded.value) {
    ...
    // Expand downward, with increased spacing from title
    ...
    const x = offset * 35
    // Adjust start position: near title but with proper spacing
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // Update pending target state (always record the latest target regardless of animation state)
        ...
        // Use debounce to delay state transition, prevent rapid flickering
        // Expand delay is shorter (50ms), collapse delay is longer (200ms) for stability
        ...
        expandDebounceTimer = setTimeout(() => {
          ...
          setTimeout(() => {
            ...
            // After animation ends, check for any new pending state
    ...
    },
    {
      ...
      // Adjust rootMargin: viewport bottom shrinks upward, requiring more scroll to trigger expand
...
onMounted(async () => {
  ...
  // Wait for DOM rendering then initialize observer
  setTimeout(() => {
    initObserver()
  }, 100)
})
...
/* Use CSS background patterns to create fixed-interval square grid */
.grid-pattern {
  position: absolute;
  top: 0;
  left: 0;
  ...
  /* Position from top-left; height changes extend downward only, grid positions unchanged */
