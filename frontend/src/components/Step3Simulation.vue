    ...
    <div class="control-bar">
      ...
      <div class="action-controls">
        ...
        >
          ...
          {{ isGeneratingReport ? 'Launching...' : 'Generate Report' }} 
...
const doStartSimulation = async () => {
  ...
  // Reset all state first to avoid interference from the previous simulation
  resetAllState()
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      startError.value = res.error || 'Launch failed'
      addLog(`✗ Launch failed: ${res.error || 'Unknown error'}`)
      emit('update-status', 'error')
    }
  } catch (err) {
    startError.value = err.message
    addLog(`✗ Launch exception: ${err.message}`)
    emit('update-status', 'error')
  } finally {
    isStarting.value = false
  }
...
// Track last round per platform to detect changes and log
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Detect round changes per platform and log accordingly
      ...
      // Detect if simulation is complete (via runner_status or platform completion flags)
      const isCompleted = data.runner_status === 'completed' || data.runner_status === 'stopped'
      
      // Extra check: if backend hasn't updated runner_status yet but platforms report completion
      // Detect via twitter_completed and reddit_completed status flags
      const platformsCompleted = checkPlatformsCompleted(data)
      
      if (isCompleted || platformsCompleted) {
        if (platformsCompleted && !isCompleted) {
          addLog('✓ All platform simulations detected as ended')
        }
...
const checkPlatformsCompleted = (data) => {
  ...
  // If at least one platform completed, check if all enabled ones have
  // Determine if platform is enabled via actions_count > 0 or if it was running
...
const fetchRunStatusDetail = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Do not auto-scroll, let user freely browse the timeline
...
const handleNextStep = async () => {
  ...
  addLog('Initiating report generation...')
