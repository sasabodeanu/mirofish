...
Runs Twitter and Reddit simulations simultaneously, reading from the same config file
...
Usage:
    ...
    python run_parallel_simulation.py --config simulation_config.json --no-wait  # close immediately on completion
...
if sys.platform == 'win32':
    ...
    # Reconfigure stdout to UTF-8 (fix garbled characters on Windows console)
    if hasattr(sys.stdout, 'reconfigure'):
        sys.stdout.reconfigure(encoding='utf-8', errors='replace')
    if hasattr(sys.stderr, 'reconfigure'):
        sys.stderr.reconfigure(encoding='utf-8', errors='replace')
    
    # Force set default encoding (affects open() function default encoding)
    ...
    def _utf8_open(file, mode='r', buffering=-1, encoding=None, errors=None, 
        ...
        This fixes issues with third-party libraries (like OASIS) reading files without specifying encoding
        """
...
# Add filter immediately at module load time, before camel code executes
...
# Twitter available actions (excludes INTERVIEW; INTERVIEW can only be triggered via ManualAction)
TWITTER_ACTIONS = [
...
# Reddit available actions (excludes INTERVIEW; INTERVIEW can only be triggered via ManualAction)
REDDIT_ACTIONS = [
    ActionType.LIKE_POST,
...
class ParallelIPCHandler:
    ...
    async def handle_interview(self, command_id: str, agent_id: int, prompt: str, platform: str = None) -> bool:
        ...
        Args:
            ...
            platform: specify platform (optional)
                ...
                - None/not specified: interview both platforms simultaneously and return combined results
            
        Returns:
            Returns True on success, False on failure
        """
        # If platform specified, interview only that platform
    ...
    async def handle_batch_interview(self, command_id: str, interviews: List[Dict], platform: str = None) -> bool:
        ...
        Args:
            ...
            platform: default platform (can be overridden by each interview item)
                - "twitter": interview Twitter platform only
                - "reddit": interview Reddit platform only
                - None/not specified: each Agent interviewed on both platforms simultaneously
        """
        # Group by platform
        twitter_interviews = []
        reddit_interviews = []
    ...
    async def process_commands(self) -> bool:
        ...
        if command_type == CommandType.INTERVIEW:
            ...
            self.send_response(command_id, "completed", result={"message": "Environment closing soon"})
            return False
...
def fetch_new_actions_from_db(
    ...
    try:
        ...
        for rowid, user_id, action, info_json in cursor.fetchall():
            ...
            # Simplify action_args, keep only key fields (keep full content, not truncated)
            simplified_args = {}
            ...
            # Add context info (post content, usernames, etc.)
...
def _enrich_action_context(
    ...
    """
    Supplement action with context information (post content, username, etc.)
    ...
    try:
        ...
        # Failure to add context does not affect main flow
        print(f"Failed to add action context: {e}")
...
def create_model(config: Dict[str, Any], use_boost: bool = False):
    ...
    Supports dual LLM configuration for parallel simulation acceleration:
    ...
    If boost LLM is configured, different platforms can use different API providers during parallel simulation to improve concurrency.
...
async def run_twitter_simulation(
    ...
    for round_num in range(total_rounds):
        ...
        if not active_agents:
            # Record round end even with no active agents (actions_count=0)
            if action_logger:
        ...
        # Get actually executed actions from database and record
    ...
    # Note: do not close environment; keep it open for Interview use
    ...
    log_info(f"Simulation loop complete! Duration: {elapsed:.1f}s, total actions: {total_actions}")
...
async def main():
    ...
    parser.add_argument(
        ...
        action='store_true',
        default=False,
        help='Close simulation environment immediately on completion, do not enter command-waiting mode'
    )
    
    args = parser.parse_args()
    
    # Create shutdown event at start of main function, ensuring the entire program can respond to exit signals
    ...
    log_manager.info("=" * 60)
    log_manager.info(f"Simulation loop complete! Total duration: {total_elapsed:.1f}s")
...
def setup_signal_handlers(loop=None):
    ...
    When termination signal is received:
    1. Notify asyncio loop to exit waiting
    2. Allow program to cleanly release resources (close database, environment, etc.)
    3. Then exit
    """
    def signal_handler(signum, frame):
        ...
        if not _cleanup_done:
            _cleanup_done = True
            # Set event to notify asyncio loop to exit (letting loop clean up resources)
            if _shutdown_event:
                _shutdown_event.set()
        
        # Do not call sys.exit() directly; let asyncio loop exit normally and clean up resources
