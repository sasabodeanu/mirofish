"""
Action logger
Records each Agent's actions in OASIS simulation for backend monitoring
...
class ActionLogger:
    """
    Action logger (backward compatibility interface)
...
# Global log instance (backward compatibility interface)
_global_logger: Optional[ActionLogger] = None


def get_logger(log_path: Optional[str] = None) -> ActionLogger:
    """Get global log instance (backward compatibility interface)"""
