...
def optimize_interview_prompt(prompt: str) -> str:
    """
    Optimize an interview prompt by adding a prefix to prevent the Agent from calling tools
...
def create_simulation():
    ...
    Note: Parameters like max_rounds are intelligently generated by the LLM, no manual setting needed
    
    Request (JSON):
        {
            "project_id": "proj_xxxx",      // required
            "graph_id": "mirofish_xxxx",    // optional, uses project graph_id if not provided
...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    ...
    Note: Run scripts (run_*.py) remain in backend/scripts/, not copied to simulation directory
    ...
    # Required files list (scripts excluded; scripts are in backend/scripts/)
    required_files = [
        "state.json",
...
def prepare_simulation():
    ...
    Features:
    - Automatically detects completed preparation work to avoid regeneration
    ...
    try:
        ...
        # Check if already prepared (avoid regeneration)
        ...
        # This allows the frontend to immediately see the expected total Agent count after calling prepare
        try:
        ...
        return jsonify({
            ...
            "data": {
                ...
                "task_id": task_id,
                "status": "preparing",
                "message": "Preparation task started, query progress at /api/simulation/prepare/status",
...
def get_prepare_status():
    ...
    2. Check for completed preparation via simulation_id
    ...
    try:
        ...
        # If simulation_id provided, check if already prepared first
        if simulation_id:
            is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
            if is_prepared:
                return jsonify({
            ...
            if simulation_id:
                ...
                return jsonify({
                    ...
                    "data": {
                        ...
                        "status": "not_started",
                        "progress": 0,
                        "message": "Not yet started, please call /api/simulation/prepare to begin",
                        "already_prepared": False
                    }
                })
            return jsonify({
...
def get_simulation_history():
    ...
    Used for homepage history display; returns simulation list with project name, description, and other rich info
    ...
    try:
        ...
        for sim in simulations:
            ...
            if run_state:
                ...
                # Use user-set total_rounds, or fallback to recommended rounds
            ...
            # Get associated report_id (find most recent report for this simulation)
...
def get_simulation_profiles_realtime(simulation_id: str):
    ...
    - Reads file directly without going through SimulationManager
    - Suitable for real-time viewing during generation
    - Returns extra metadata (e.g. file modification time, whether still generating)
    ...
    try:
        ...
        # Check if still generating (via state.json)
        is_generating = False
        total_expected = None
...
def download_simulation_script(script_name: str):
    """
    Download simulation run script file (general scripts in backend/scripts/)
...
def start_simulation():
    ...
    About the force parameter:
        - When enabled, if simulation is running or completed, it will be stopped and run logs cleared first
        ...
        - Use this when you need to re-run the simulation

    About enable_graph_memory_update:
        - When enabled, all Agent activities during simulation (posting, commenting, liking, etc.) will be updated to the Zep Graph in real time
    ...
    try:
        ...
        # Smart status handling: if preparation is done, allow restart
        if state.status != SimulationStatus.READY:
            # Check if preparation is complete
            is_prepared, prepare_info = _check_simulation_prepared(simulation_id)

            if is_prepared:
                # Preparation done, check if process is already running
                if state.status == SimulationStatus.RUNNING:
                    ...
                    if run_state and run_state.runner_status.value == "running":
                        ...
                        if force:
                            ...
                            return jsonify({
                                "success": False,
                                "error": f"Simulation is running; please call /stop to stop it first, or use force=true to force restart"
                            }), 400
                ...
                # Process does not exist or ended; reset status to ready
                logger.info(f"Simulation {simulation_id} preparation done, resetting status to ready (was: {state.status.value})")
...
def get_run_status_detail(simulation_id: str):
    ...
    try:
        ...
        # recent_actions shows only the latest round from both platforms
...
def get_agent_stats(simulation_id: str):
    ...
    Used for frontend Agent activity ranking and action distribution display
    """
    try:
...
def interview_agent():
    ...
    Note: This feature requires the simulation environment to be in running state (enters command-waiting mode after completing simulation loop)
    ...
    Request (JSON):
        {
            ...
            "platform": "twitter",             // optional, specify platform (twitter/reddit)
                                               // if not specified: both platforms interviewed simultaneously in dual-platform simulation
    ...
    try:
        ...
        if not SimulationRunner.check_env_alive(simulation_id):
            return jsonify({
                "success": False,
                "error": "Simulation environment is not running or has been closed. Please ensure simulation has completed and entered command-waiting mode."
            }), 400
    ...
    Request (JSON):
        {
            ...
            "interviews": [                    // required, interview list
                ...
                {
                    "agent_id": 1,
                    "prompt": "What do you think about B?"  // uses default platform if not specified
                }
            ],
            "platform": "reddit",              // optional, default platform (overridden by each item)
                                               // Not specified: interview each Agent on both platforms in dual-platform simulation
    ...
    Request (JSON):
        {
            ...
            "agent_id": 0,                 // optional, get only this Agent's history
...
def close_simulation_env():
    ...
    while this endpoint causes the simulation to gracefully close the environment and exit.
