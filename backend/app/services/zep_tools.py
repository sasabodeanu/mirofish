...
1. InsightForge (deep insight retrieval) - Most powerful hybrid retrieval; auto-generates sub-questions and retrieves from multiple dimensions
...
class AgentInterview:
    ...
    def to_text(self) -> str:
        ...
        # Display complete agent_bio, no truncation
        ...
        if self.key_quotes:
            ...
            for quote in self.key_quotes:
                ...
                # Filter out noise containing question numbers (Q1-9)
                skip = False
                for d in '123456789':
...
class ZepToolsService:
    ...
    4. interview_agents - In-depth interview (interview simulation Agents for multi-perspective views)
    
    [Basic Tools]
    - search_graph - graph semantic search
    - get_all_nodes - get all graph nodes
    - get_all_edges - get all graph edges (with time information)
    ...
    def _local_search(
        ...
        def match_score(text: str) -> int:
            """Calculate matching score between text and query"""
            if not text:
                return 0
    ...
    def get_all_edges(self, graph_id: str, include_temporal: bool = True) -> List[EdgeInfo]:
        """
        Get all edges from the graph (paginated, with time information)
    ...
    def get_simulation_context(
        ...
        Args:
            ...
            limit: count limit per type of information
    ...
    def insight_forge(
        ...
        The most powerful hybrid retrieval function, automatically decomposes questions and searches multi-dimensionally:
        1. Use LLM to decompose the question into multiple sub-questions
        2. Perform semantic search for each sub-question
        3. Extract relevant entities and get their detailed information
        ...
        # Step 3: Extract relevant entity UUIDs from edges, get only these entities (not all nodes)
        entity_uuids = set()
        ...
        # Get details for all relevant entities (no count limit, complete output)
        entity_insights = []
        ...
        for uuid in list(entity_uuids):  # process all entities, no truncation
            if not uuid:
                continue
            try:
                # Get information for each relevant node individually
    ...
    def _generate_sub_queries(
        ...
        Decompose complex questions into multiple independently searchable sub-questions
        """
...
Return a JSON-formatted list of sub-questions."""
    ...
    def panorama_search(
        ...
        This tool is suitable for scenarios requiring understanding of the full picture of events and tracking evolution processes.
        
        Args:
            graph_id: graph ID
            query: search query (for relevance ranking)
            include_expired: whether to include expired content (default True)
            limit: number of results to return (count limit)
    ...
    def interview_agents(
        ...
        4. Call /api/simulation/interview/batch endpoint for real interviews (both platforms simultaneously)
        ...
        - Need to get real responses from simulation Agents (not LLM simulation)
        ...
        INTERVIEW_PROMPT_PREFIX = (
            ...
            "6. Answers must have substantial content, at least 2-3 sentences per question\n\n"
        )
        ...
        try:
            ...
            for i, agent_idx in enumerate(selected_indices):
                ...
                # Clean response text: remove markers, numbers, Markdown noise, etc.
                ...
                # Strategy 2 (supplementary): long text within correctly paired Chinese quotes「」
                if not key_quotes:
            ...
            result.summary = f"Interview failed: {str(e)}. Simulation environment may have closed, ensure OASIS environment is running."
            return result
        except Exception as e:
    ...
    def _clean_tool_call_response(response: str) -> str:
        """Clean JSON tool call wrapping from Agent response and extract actual content"""
    ...
    def _load_agent_profiles(self, simulation_id: str) -> List[Dict[str, Any]]:
        ...
        # Preferentially try reading Reddit JSON format
...
2. Point out consensus and divergences in viewpoints
...
- May use **bold** to highlight keywords, but no other Markdown syntax"""
