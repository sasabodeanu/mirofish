...
CHINA_TIMEZONE_CONFIG = {
    ...
    "activity_multipliers": {
        "dead": 0.05,      # Almost no activity in the early morning
...
class AgentActivityConfig:
    ...
    # Response speed (reaction delay for trending events, unit: simulated minutes)
...
class TimeSimulationConfig:
    ...
    # Time per round (simulated minutes) - default 60 minutes (1 hour), to speed up time flow
...
class PlatformConfig:
    ...
    # Viral spread threshold (interactions needed to trigger viral spread)
...
class SimulationConfigGenerator:
    ...
    to automatically generate optimal simulation parameters
    ...
    ENTITIES_PER_TYPE_DISPLAY = 20       # Entities per type to display
...
### Basic principles (for reference; adjust flexibly based on event and participant group):
- User group is Chinese; should follow Beijing time daily schedule habits
- Almost no activity 0-5 AM (activity multiplier 0.05)
- Gradually active 6-8 AM (activity multiplier 0.4)
- Moderately active during work hours 9-18 (activity multiplier 0.7)
- Peak hours 19-22 PM (activity multiplier 1.5)
- Activity declines after 23 PM (activity multiplier 0.5)
- General pattern: low activity late night, gradual increase morning, moderate during work, peak evening
...
- work_hours (int array): Work period
- reasoning (string): Brief explanation of why this configuration was chosen"""

        system_prompt = "You are a social media simulation expert. Return pure JSON format; time configuration should match Chinese daily schedule habits."
    ...
    def _parse_time_config(self, result: Dict[str, Any], num_entities: int) -> TimeSimulationConfig:
        """Parse time config result and validate agents_per_hour values do not exceed total agent count"""
        # Get raw values
...
Example: official statements should be posted by Official/University type, news by MediaOutlet, student opinions by Student.
...
{{
    ...
    def _assign_initial_post_agents(
        ...
        """
        Assign appropriate poster Agent for initial posts
...
- **Official institutions** (University/GovernmentAgency): Low activity (0.1-0.3), active work hours (9-17), slow response (60-240 min), high influence (2.5-3.0)
...
{{
    "agent_configs": [
        ...
        system_prompt = "You are a social media behavior analysis expert. Return pure JSON; configuration should match Chinese daily schedule habits."
    ...
    def _generate_agent_config_by_rule(self, entity: EntityNode) -> Dict[str, Any]:
        """Generate single Agent config using rules (Chinese daily schedule)"""
        entity_type = (entity.get_entity_type() or "Unknown").lower()
        
        if entity_type in ["university", "governmentagency", "ngo"]:
            # Official institutions: active work hours, low frequency, high influence
            return {
                "activity_level": 0.2,
                "posts_per_hour": 0.1,
